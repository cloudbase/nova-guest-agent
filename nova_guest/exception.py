...
from nova_guest.i18n import _, _LE  # noqa
...
class NotAuthorized(NovaGuestException):
...
class Conflict(NovaGuestException):
    message = _("Conflict")
    code = 409
    safe = True
...
class Invalid(NovaGuestException):
...
class InvalidDetectedOSParams(NovaGuestException):
...
class NotFound(NovaGuestException):
...
class Duplicate(NovaGuestException):
    safe = True
...
class ConnectionValidationException(NovaGuestException):
    safe = True


class SchemaValidationException(NovaGuestException):
    safe = True
